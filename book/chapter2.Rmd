...
## Importing .csv, .sav and .spss file as data frames: read

To import a data set, you first need to activate the correct package (these are `readr` (for csv files, although this is part of the tidyverse core and is thus installed by using `library("tidyverse")`), `foreign` (for SPSS files), or `readxl` (for excel files, also part of the tidyverse, but not of the core), depending on the type of data you want to import). Next to that, you need to know the location of the data set on your computer. Suppose the location of your data file called 'dataset_4' is: "C:/My Documents". The data set can be loaded into the *global environment* with the following commands:
...
data <- read.csv("dataset_4.csv", sep = ",")

# load an spss file with the packave foreign
data <- read.spss("dataset_4.sav", to.data.frame = TRUE)
...
## Viewing or creating subsets of a data frame: select, filter and the use the assignment operator <-
...
# Using cuolumn number to select variables:
gss_cat_social <- gss_cat %>% 
  select(1:5, 7, 8)
...
# or, for a standardized variable, simply use
gss_cat <- gss_cat %>%
...
# This means: in the data set gss_cat, replace the existing variable relig (which contains contains cases with the word 'Don't know') with a new variable relig, in which this group is declared missing (and gets an NA).
```
...
## Adding observations to an existing data file: add_row
